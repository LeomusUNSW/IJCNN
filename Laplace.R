...
# ========== Multivariate (independent) Laplace parameters ==========
# 每个类在两个维度上独立 Laplace：loc=(mu1, mu2), scale=(b1, b2)
lap1 <- list(loc = c(-1, 0), scale = c(1.0, 0.8))  # Component 1
lap2 <- list(loc = c( 1, 0), scale = c(1.0, 0.8))  # Component 2

# ---- Univariate Laplace: density & sampler ----
dlaplace1 <- function(x, loc = 0, scale = 1, log = FALSE) {
  z <- -abs(x - loc) / scale - log(2*scale)
  if (log) z else exp(z)
}
# 生成 n 个一维 Laplace：loc + scale * (E1 - E2)，E1,E2~Exp(1)
rlaplace1 <- function(n, loc = 0, scale = 1) {
  loc + scale * (rexp(n, 1) - rexp(n, 1))
}

# ---- 2D Laplace（独立维度）：密度与采样 ----
# 密度为两维独立 Laplace 密度的乘积
dlaplace2_prod <- function(y, loc, scale, log = FALSE) {
  l1 <- dlaplace1(y[1], loc[1], scale[1], log = TRUE)
  l2 <- dlaplace1(y[2], loc[2], scale[2], log = TRUE)
  if (log) (l1 + l2) else exp(l1 + l2)
}
# 采样：各维独立
rlaplace2_prod <- function(n, loc, scale) {
  cbind(
    rlaplace1(n, loc[1], scale[1]),
    rlaplace1(n, loc[2], scale[2])
  )
}

# ---- Posterior probability under the true Laplace mixture ----
posterior_prob <- function(y, pi, lap1, lap2) {
  dens1 <- dlaplace2_prod(y, lap1$loc, lap1$scale, log = FALSE)
  dens2 <- dlaplace2_prod(y, lap2$loc, lap2$scale, log = FALSE)
  f1 <- dens1 * pi[1]
  f2 <- dens2 * pi[2]
  f1 / (f1 + f2)
}

# ---- Margin confidence function (based on true Laplace posterior) ----
margin_conf <- function(y) {
  tau1 <- posterior_prob(y, pi, lap1, lap2)
  abs(2*tau1 - 1)
}

# ---- Generate data with margin confidence in [0, 0.6] (hard set) ----
sample_data <- function(n, pi, lap1, lap2) {
  out <- matrix(NA, nrow = 0, ncol = 3)
  while (nrow(out) < n) {
    z <- sample(1:2, 1, prob = pi)
    if (z == 1) {
      y <- rlaplace2_prod(1, lap1$loc, lap1$scale)
    } else {
      y <- rlaplace2_prod(1, lap2$loc, lap2$scale)
    }
    mc <- margin_conf(c(y[1], y[2]))
    if (mc >= 0 && mc <= 0.6) {
      out <- rbind(out, c(y, z))
    }
  }
...
# Generate the data
data <- sample_data(n, pi, lap1, lap2)
...
## =========================
## 1) Building blocks for ECM (still assuming Gaussian mixture)
## =========================
...
pack_theta <- function(pis, mus, Sigmas) {
  ...
  log_diag <- function(Lv){
    idx <- cumsum(1:d); Lv[idx] <- log(Lv[idx]); Lv
  }
  L1v <- log_diag(L1v); L2v <- log_diag(L2v)
...
unpack_theta <- function(theta_vec, d) {
  ...
  toL <- function(Lv){
    L <- matrix(0,d,d); L[lower.tri(L, diag=TRUE)] <- Lv
    for(i in 1:d) L[i,i] <- exp(L[i,i]); L
  }
...
ECM_GMM_MAR <- function(Y, z_obs, lambda_init = 0.5, estimate_lambda = TRUE,
  ...
  stopifnot(K == 2)
  m <- ifelse(is.na(z_obs), 1L, 0L)  # missing indicator
  ...
  for (t in 1:max_iter) {
    ...
    tau <- comp$tau; delta <- comp$delta; delta2 <- delta^2
    if (any(m == 0L)) {
      tau[cbind(which(m==0L), z_obs[m==0L])] <- 1
      tau[cbind(which(m==0L), 3 - z_obs[m==0L])] <- 0
    }
...
ECM_GMM_MAR_LOGIT <- function(Y, z_obs,
  ...
  list(pis=pis, mus=mus, Sigmas=Sigmas,
       alpha=c(alpha0=alpha0, alpha1=alpha1),
       tau=tau, delta=delta, z_impute=z_impute, loglik=loglik_trace)
}
