...
# Missing-mechanism negative log-likelihood for alpha (given lambda, delta2) -- AO
...
# ECM main for Logit missingness (no lambda)
ECM_GMM_MAR_LOGIT <- function(Y, z_obs,
  ...
  for (t in 1:max_iter) {
    ...
    # CM-step 1: strict joint update of mixture theta under logit miss
    theta0 <- pack_theta(pis, mus, Sigmas)
    opt_theta <- optim(
      par = theta0,
...
z_obs <- data$label           # Observed labels with NA for missing
z_true <- data$label_true     # True labels
y_true01 <- as.integer(z_true == 1)

# ---- ECM (AO link) ----
fit <- ECM_GMM_MAR(Y, z_obs, lambda_init = 0.5, estimate_lambda = TRUE, max_iter = 500, tol = 1e-6, verbose = FALSE)
p_hat_ecm <- fit$tau[,1]
...
## =========================
## 4b) One combined ROC plot (all three methods, academic colors)
## =========================
...
extract_metrics <- function(p_hat, y_true, roc_obj, thr_fixed=0.5) {
  ...
  F1_opt <- ifelse(Precision_opt+Recall_opt==0, NA, 2*Precision_opt*Recall_opt/(Precision_opt+RecallOpt <- Precision_opt+Recall_opt))
...
# fix a tiny typo in F1_opt line above
extract_metrics <- function(p_hat, y_true, roc_obj, thr_fixed=0.5) {
  auc_val <- as.numeric(auc(roc_obj))
  eps <- 1e-12
  LogLoss <- -mean(y_true*log(pmax(p_hat,eps)) + (1-y_true)*log(pmax(1-p_hat,eps)))
